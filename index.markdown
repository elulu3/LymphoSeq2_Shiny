...
Outputs from the functions **VDJ Chord Diagram** and **Gene Frequency Word Cloud** are only available for download in a local deployment of the app as an .html file, which can be opened with any web browser. Interactivity of the output is also maintained in the file. When the download button is clicked, the .html file will automatically be saved in the same directory as the app's source code (rather than downloading the file in the device's */Downloads* directory as the other functions do).
